"""Run Groups — durable multi-agent orchestration groups."""
...
def _file_rows(data: Any) -> list[dict[str, Any]]:
    data = _unwrap(data)
...
def _run_body(entry: dict[str, Any]) -> dict[str, Any]:
    return _body(
        instruction=_pick(entry, "instruction"),
        message=_pick(entry, "message"),
        target_kind=_pick(entry, "target_kind", "targetKind"),
        target_id=_pick(entry, "target_id", "targetId"),
        runtime_id=_pick(entry, "runtime_id", "runtimeId"),
        ...
        parent_run_id=_pick(entry, "parent_run_id", "parentRunId"),
        orchestration_role=_pick(entry, "orchestration_role", "orchestrationRole"),
        skip_agent_runtime_profile=_pick(
            entry, "skip_agent_runtime_profile", "skipRuntimeProfile"
        ),
        execution_packet=_pick(entry, "execution_packet", "executionPacket"),
        expected_outputs=_pick(entry, "expected_outputs", "expectedOutputs"),
...
class RunGroups:
    ...
    def get(self, group_id: str, *, include_runs: bool = False) -> dict[str, Any]:
        ...
        return _unwrap(self._t.request("GET", f"/run-groups/{group_id}", params=params))
    ...
    def dispatch(
        ...
        return _unwrap(
            self._t.request(
                "POST",
                f"/run-groups/{group_id}/dispatch",
    ...
    def cancel(self, group_id: str) -> dict[str, Any]:
        return _unwrap(
            self._t.request("POST", f"/run-groups/{group_id}/cancel", json_body={})
        )

    def activity(self, group_id: str) -> list[dict[str, Any]]:
        data = _unwrap(self._t.request("GET", f"/run-groups/{group_id}/activity"))
        if isinstance(data, list):
            return data
        if isinstance(data, dict):
    ...
    def stream_activity(self, group_id: str) -> Iterator[dict[str, Any]]:
        yield from self._t.stream_sse(f"/run-groups/{group_id}/activity")

    def files(self, group_id: str) -> list[dict[str, Any]]:
        ...
        for run in group.get("runs") or []:
            if not isinstance(run, dict) or not run.get("id"):
                continue
            run_id = str(run["id"])
            for file in _file_rows(self._t.request("GET", f"/runs/{run_id}/files")):
                files.append({**file, "run_id": file.get("run_id") or run_id})
        return files
        ...
        *,
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(f"Timed out waiting for run group {group_id}")
            time.sleep(min(poll_interval, max(0.0, deadline - time.monotonic())))


class AsyncRunGroups:
    """Async Run Groups API."""
    ...
    async def get(self, group_id: str, *, include_runs: bool = False) -> dict[str, Any]:
        params = {"include": "runs"} if include_runs else None
        return _unwrap(
            await self._t.request("GET", f"/run-groups/{group_id}", params=params)
        )
    ...
    async def cancel(self, group_id: str) -> dict[str, Any]:
        return _unwrap(
            await self._t.request("POST", f"/run-groups/{group_id}/cancel", json_body={})
        )

    async def activity(self, group_id: str) -> list[dict[str, Any]]:
        data = _unwrap(await self._t.request("GET", f"/run-groups/{group_id}/activity"))
        if isinstance(data, list):
            return data
        if isinstance(data, dict):
            activity = data.get("activity") or data.get("items")
    ...
    async def stream_activity(self, group_id: str) -> AsyncIterator[dict[str, Any]]:
        async for event in self._t.stream_sse(f"/run-groups/{group_id}/activity"):
            yield event

    async def files(self, group_id: str) -> list[dict[str, Any]]:
        ...
        for run in group.get("runs") or []:
            if not isinstance(run, dict) or not run.get("id"):
                continue
            run_id = str(run["id"])
            for file in _file_rows(await self._t.request("GET", f"/runs/{run_id}/files")):
                files.append({**file, "run_id": file.get("run_id") or run_id})
        return files
